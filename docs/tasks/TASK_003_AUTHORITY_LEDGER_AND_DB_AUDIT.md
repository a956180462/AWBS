...
- `refs/awbs/trusted` 指向当前 AWBS 认证数据库头，但它不能单独作为事实源；它指向的 commit 必须被 sealed ledger head entry 验证。
...
- ledger hash chain 是否从头到尾可重算。
- trusted commit 的父提交、commit message、变更路径和最终路径内容 hash 是否匹配 ledger head entry。
