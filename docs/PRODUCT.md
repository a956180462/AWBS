...
只有 authority 承认的 changeset apply 才能推进 trusted chain。`refs/awbs/trusted` 本身不是单独事实源；它指向的 commit 必须能被 sealed ledger head entry 验证，包括父提交、commit message、变更路径和最终路径内容 hash。
...
- trusted commit 与 ledger head entry 绑定校验。
